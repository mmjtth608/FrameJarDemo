package com.tb.tclass;

import com.tb.iml.Headable;
...
public class SimpleHeader implements Headable {
	...
	public SimpleHeader(Context context) {
		...
		int fontSize = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_SP, 20, context.getResources()
						.getDisplayMetrics());
		...
		mHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
				45, context.getResources().getDisplayMetrics());
		mPointRadius = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
				2.5f, context.getResources().getDisplayMetrics());
	...
	public boolean draw(Canvas canvas, int left, int top, int right, int bottom) {
		...
		case STATE_RELEASE:
			...
			for (int i = 0; i < mPice; i++) {
				...
				float radius = width / 2 - radiusParam
		...
		case STATE_FAIL:
			...
			if (time < 30) {
				...
				for (int i = 0; i < mPice; i++) {
					...
					float y;
					if (offset < height) {
						y = (float) (offset - height / 2 + radius
								* Math.sin(angle));
					} else {
						y = (float) (offset / 2 + radius * Math.sin(angle));
					}
				...
				String text = mMsg != null ? mMsg
						: mState == STATE_SUCCESS ? "加载成功" : "加载失败";
				float y;
				if (offset < height) {
					y = offset - height / 2;
				} else {
	...
	public void toastResultInOtherWay(Context context, int state) {
		if (state == Headable.STATE_SUCCESS) {
			...
			Toast.makeText(context, mMsg != null ? mMsg : "加载失败",
